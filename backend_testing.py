...
class BackEndTests:
    ...
    def check_post(self):
        requests.post(f'http://127.0.0.1:5001/users/{self.user_id}', json={"user_name": self.name})

    def get_user(self):
        res = requests.get(f'http://127.0.0.1:5001/users/{self.user_id}')
    ...
    def clean_user(self):
        res = requests.delete(f'http://127.0.0.1:5001/users/{self.user_id}')
        assert res.status_code == 200
